...
class MusicList extends BaseComponent {
    ...
    render() {
        this.music = this.props.music;
        return (
            <TouchableOpacity onPress={() => {
                this.props.navigation.push('musicView', {
                    itemId : this.music.id,
                    item : this.music
                })
            }}>
                <View style={{flexDirection:'row',padding:10,borderBottomWidth:0.6,borderBottomColor:'#EEEEEE'}}>
                    <FastImage source={{ uri: this.music.cover }} style={{width:60,height:60,borderRadius:30}} />
                    <View style={{flex:1, marginLeft:10, marginTop:5}}>
                        <Text style={{fontWeight:'bold',marginTop: 10,color:'grey'}}>{this.music.title}</Text>
                    </View>
                </View>
            </TouchableOpacity>
        );
