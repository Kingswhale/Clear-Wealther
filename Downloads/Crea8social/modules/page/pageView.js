...
class PageView extends ProfileBaseComponent {
    constructor(props) {
        super(props);
    ...
    finishedEdit(res) {
        this.updateState({itemDetails: res.page});
    }
    ...
    loadItemDetail() {
       ...
       // console.log("page-id" + this.props.navigation.getParam("itemId"));
        if (item === undefined) {
            Api.get("page/details", {
                userid : this.props.userid,
                page_id : this.props.navigation.getParam("itemId")
            }).then((res) => {
                if (res.status === 1) {
                    this.updateState({
                        itemDetails : res,
                ...
                {this.state.itemDetails !== null ? (
                    <View style={{flex:1,backgroundColor: 'white'}}>
                        <Modal
                            ...
                            <PageCreate navigation={this.props.navigation} type="edit" item={this.state.itemDetails} itemId={this.state.itemDetails.id} component={this}/>
                        </Modal>
                        <Animated.View style={{
                                            ...
                                            <View style={{position: 'absolute', bottom: 0,width:'100%',padding:20,flexDirection: 'row'}}>
                                                ...
                                                <View style={{flexDirection: 'column',flex:1, marginLeft:10,marginTop:0}}>
                                                    <Text style={{fontSize:20,color:'white',marginTop:10}}>
                                                        {this.state.itemDetails.title} {this.state.itemDetails.verified ? (
                                                    ...
                                                    <View style={{flexDirection: 'row', marginTop:10}}>
                                                        ...
                                                        {this.state.itemDetails.has_like ? (
                                                            <Button
                                                                onPress={() => {
                                                                    this.updateState({processing: true});
                                                                    Api.get("page/like", {
                                                                        userid : this.props.userid,
                                                                        page_id : this.state.itemDetails.id,
                                                                        action : 0
                                                                    }).then((res) => {
                                                                        let likes = parseInt(this.state.itemDetails.likes) - 1;
                                                                        let details = {...this.state.itemDetails, has_like: false,likes:likes};
                                                                ...
                                                                <Icon name="md-thumbs-up" style={{marginLeft:10}}/>
                                                                <Text style={{marginLeft:5,marginRight:15}}>{this.lang.t('unlike')}</Text>
                                                            </Button>
                                                        ) : (
                                                            <Button
                                                                onPress={() => {
                                                                    ...
                                                                    Api.get("page/like", {
                                                                        ...
                                                                        let likes = parseInt(this.state.itemDetails.likes) + 1;
                                                                        let details = {...this.state.itemDetails, has_like: true,likes:likes};
                                                                        this.updateState({processing:false,itemDetails: details});
                                                                    }).catch((e) => {
                                                                        this.updateState({processing: false});
                                                                    })
                                                                ...
                                                                light small style={{borderRadius:5,marginTop:10}}>
                                                                <Icon name="md-thumbs-up" style={{marginLeft:10}}/>
                                                                <Text style={{marginLeft:5,marginRight:15}}>{this.lang.t('like')}</Text>
                                                            </Button>
                                                        )}

                                                        <Text style={{fontSize:17,marginLeft:15, color:'white',marginTop:12}}>
                                                            {this.state.itemDetails.likes} {this.lang.t('people_likes')}
                                                            </Text>
                                                    </View>
                            ...
                            <Tabs
                                ...
                                <Tab style={{backgroundColor: '#DEDCDD'}}  heading={this.lang.t('timeline')} >
                                    <FeedComponent
                                        ...
                                        entityId={this.state.itemDetails.id}
                                        entityType="page"
                                        toUserid=""
                                        canPost={this.state.itemDetails.is_admin}
                                        showWelcome={false}
                                        style={{flex: 1}} component={this}/>
                                </Tab>

                                <Tab style={{backgroundColor: 'white'}}  heading={this.lang.t('about')} >
                                    <ScrollView>
                                        <View style={{backgroundColor:'white', padding:10}}>
                                            <Text style={{fontWeight:'bold', color:'black'}}>{this.lang.t('page_description')}</Text>
                                            <Text style={{marginTop:20}}>{this.state.itemDetails.description}</Text>
                                        </View>
                                    </ScrollView>
                            ...
                            <View>
                                <ListItem noBorder icon onPress={()=>{
                                    ...
                                    ImagePicker.openPicker({
                                        ...
                                        form.append("page_id", this.state.itemDetails.id);
                                        form.append('image', {
                                            uri: image.path,
                                        ...
                                        Api.post("page/logo", form).then((res) => {
                                            this.updateState({processing : false});
                                            if (res.status === 1) {
                                ...
                                <ListItem noBorder icon onPress={()=>{
                                    ...
                                    ImagePicker.openPicker({
                                        ...
                                        Api.post("page/cover", form).then((res) => {
                                            this.updateState({processing : false});
                                            if (res.status === 1) {
                                                let image = res.data_one;
                                                let itemDetails = this.state.itemDetails;
                                ...
                                <ListItem noBorder icon onPress={()=>{
                                    ...
                                    <Left><Icon active name="pencil" type="SimpleLineIcons" style={{color:'#2196F3', fontSize: 15}} /></Left>
                                    <Body><Text>{this.lang.t('edit_page')}</Text></Body>
                                </ListItem>
                                ...
                                <ListItem noBorder icon onPress={()=>{
                                    ...
                                    <Body><Text>{this.lang.t('delete_page')}</Text></Body>
                                </ListItem>
                            </View>
                        ) : null}
