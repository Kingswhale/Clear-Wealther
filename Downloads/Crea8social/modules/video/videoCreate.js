...
class VideoCreate extends BaseComponent {
    constructor(props) {
        super(props);
    ...
    submitData() {
        ...
        if (this.state.video !== null) {
            formData.append('video', {
                uri: this.state.video.path,
                type: this.state.video.mime, // or photo.type
                name: this.state.video.path
            });

        }
        if (this.type === 'new') {
            Api.post("video/create",formData).then((res) => {
                this.updateState({
                    processing: false
                });
                if (res.status === 1) {
                    ...
                    Toast.show({
                        text: this.lang.t('video_added_success'),
                        type : 'success'
                    });
                } else {
                    Toast.show({
            ...
            Api.post("video/edit",formData).then((res) => {
                this.updateState({
                    processing: false
                });
                if (res.status === 1) {
    ...
    loadItemCategories() {
        Api.get("videos/get/categories", {userid : this.props.userid}).then((res) => {
                    ...
                    <Container>
                        ...
                        <Header noShadow hasTabs>
                            ...
                            <Body>
                                <Text style={{color: 'white'}}>
                                    {this.type === 'new' ? this.lang.t('new_video').toUpperCase() : this.lang.t('edit_video').toUpperCase()}
                                    </Text>
                            </Body>
                            <Right>
                                <Button
                        ...
                        <Content >
                            <Form style={{flex: 1,margin:20}}>
                                ...
                                <Item floatingLabel>
                                    <Label>{this.lang.t('embed_video_link')}</Label>
                                    <Input onChangeText={(text) => this.updateState({link : text})} value={this.state.link} />
                                </Item>

                                <Item picker style={{marginBottom: 20,marginLeft:20}}>
                                    <Left>
                                        <Label>{this.lang.t('or_video_file')}</Label>
                                    </Left>
                                    <Right>
                                        {this.state.image === null ? (
                                            <Button onPress={() => {
                                                ...
                                                <Text style={{marginLeft:20, marginRight:20}}>{this.lang.t('choose_video')}</Text>
                                            </Button>
                                        ) : (<Button onPress={() => {
                                            ImagePicker.openPicker({mediaType : 'video'}).then(video => {
                                                this.updateState({
                                            ...
                                            <Text style={{marginLeft:20, marginRight:20,color:'white'}}>{this.lang.t('choose_video')}</Text>
                                        </Button>)}
                                    </Right>
                                </Item>
