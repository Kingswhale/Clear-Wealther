...
class PageCreate extends BaseComponent {
    constructor(props) {
        super(props);
    ...
    submitData() {
        ...
        if (this.type === 'new') {
            Api.post("page/create",formData).then((res) => {
                this.updateState({
                    processing: false
                });
                if (res.status === 1) {
                    ...
                    Toast.show({
                        text: this.lang.t('page_added_success'),
                        type : 'success'
                    });
                } else {
                    Toast.show({
            ...
            Api.post("page/edit",formData).then((res) => {
                this.updateState({
                    processing: false
                });
                if (res.status === 1) {
    ...
    loadItemCategories() {
        Api.get("page/get/categories", {userid : this.props.userid}).then((res) => {
                        ...
                        <Header noShadow hasTabs>
                            ...
                            <Body>
                                <Text style={{color: 'white'}}>
                                    {this.type === 'new' ? this.lang.t('new_page').toUpperCase() : this.lang.t('edit_page').toUpperCase()}
                                    </Text>
                            </Body>
                            <Right>
                                <Button
